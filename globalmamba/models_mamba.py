...
class PatchEmbed_freq(nn.Module):
    ...
    def forward(self, x, resolution):
        ...
        downsample = int(resolution / self.patch_size[0])
        x = self.proj(x)
        x = torch.nn.functional.interpolate(x, size=(downsample, downsample), mode="bilinear")
...
def mask_generate(img_size, num):
    out = []
    ...
    for i in reversed(range(0, num-1)):
        ...
        out.append(torch.nn.functional.pad(mask, (0, img_size - img_size // (2**i), 0, img_size - img_size // (2**i)), mode='constant', value=0))
...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        for i, mask in enumerate(self.freq_range):
            fft_sample = x_fft * mask
            ifft_sample = dct.idct_2d(fft_sample)
            ifft_sample = torch.real(ifft_sample)
            ifft_sample_patch = self.patch_embed(ifft_sample, mask.shape[0] // (2**(3-i)))
            fre_list.append(ifft_sample_patch)
        
        x = torch.cat(fre_list, dim=1)
